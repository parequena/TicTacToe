...
bool Player::checkWinner(std::array<std::uint8_t, 9> const& board)
{
   return (board[0] != 0
            && (((board[0] == board[1]) && (board[0] == board[2]))  /* 0-1-2 */
              ...
              || ((board[0] == board[4])
                && (board[0] == board[8])))) /* 0-4-8 */
     || (board[3] != 0 && board[3] == board[4]
       && board[3] == board[5]) /* 3-4-5 */
     || (board[6] != 0 && board[6] == board[7]
       && board[6] == board[8]) /* 6-7-8 */
     || (board[1] != 0 && board[1] == board[4]
       && board[1] == board[7]) /* 1-4-7 */
     || (board[2] != 0
