...
// Difficulty 0.
std::uint8_t AIPlayer::difficulty0(const std::uint8_t board[9]) const 
{
    ...
    return freePos[std::size_t(rand()) % freePos.size()]; // Return a random free position.
}

// Difficulty 1.
std::uint8_t AIPlayer::difficulty1(const std::uint8_t board[9]) const 
{
    bool empty = true;
    ...
    if(empty) return std::uint8_t(rand() % 9);
...
std::uint8_t AIPlayer::selectBox(const std::uint8_t board[9]) 
{
