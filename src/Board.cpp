...
// Ctor.
Board::Board(
  sf::RenderWindow* window, std::uint8_t nplayers, std::uint8_t difficulty)
{
...
std::uint8_t Board::play()
{
   ...
   do
   {
      do
      {
        ...
        selected >= 10 || !sf::Keyboard::isKeyPressed(sf::Keyboard::Return));
      waitUntilRelease(sf::Keyboard::Return);
      ...
      // Create the token.
      m_tokens[m_currToken++] = new Token(m_window, selected,
        static_cast<TokenType>(m_players[m_firstPlayer]->getId() - 1));
