    ...
    , m_position(0)
{
   ...
   m_texts[1] = new sf::Text(
     "N. Players: " + std::to_string(m_selection->m_nPlayers), *m_font, 30);
   m_texts[2] = new sf::Text(
     "AI Level: " + std::to_string(m_selection->m_AILevel), *m_font, 30);
   ...
   do
   {
      ...
      for (auto const [key, pFunc] : menu_ktom)
      {
         ...
         if (sf::Keyboard::isKeyPressed(key))
         {
            ...
            m_texts[m_position]->setFillColor(
              sf::Color::Black); // Set previous to Black.
            m_position = pos;    // Set m_position to actual.
            m_texts[m_position]->setFillColor(
              sf::Color::Blue); // Set actual to Blue.
      ...
      if (m_position == 1)
      {
         for (auto const [key, pFunc] : menu_ktom2)
         {
            ...
            if (sf::Keyboard::isKeyPressed(key))
            {
               m_selection->m_nPlayers
                 = pFunc(m_selection->m_nPlayers); // Call the function.
               if (m_selection->m_nPlayers >= 2)
                  m_selection->m_nPlayers = 2;
               waitUntilRelease(key);
               m_texts[1]->setString(
                 "N. Players: " + std::to_string(m_selection->m_nPlayers));
            }
         }
      }
      else if (m_position == 2)
      {
         for (auto const [key, pFunc] : menu_ktom2)
         {
            ...
            if (sf::Keyboard::isKeyPressed(key))
            {
               m_selection->m_AILevel
                 = pFunc(m_selection->m_AILevel); // Call the function.
               if (m_selection->m_AILevel >= 1)
                  m_selection->m_AILevel = 1;
               waitUntilRelease(key);
               m_texts[2]->setString(
                 "AI Level  : " + std::to_string(m_selection->m_AILevel));
            }
         }
      }
